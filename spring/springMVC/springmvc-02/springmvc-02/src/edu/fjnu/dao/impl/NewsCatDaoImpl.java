...
//		String sql = "insert into NewsCat(catId,catName, catDesc, parentId";
//		sql += " ,status ,remark ,sortNum ,isDeleted";
//		sql += " ,createBy , updateBy ,createOn ,updateOn) ";
//		sql += " values(news_cat_id.nextval,?,?,?, ?,?,?,?, ?,?,?,?)";
//
...
//
//		};
		String sql = "insert into NewsCat(catId,catName, catDesc, parentId) values(news_cat_id.nextval,?,?,?)";
	...
	public NewsCat load(Long id) {
		...
		String sql = "select * from NewsCat where catId=? and isdeleted!=1";
		Object[] params = { id };
	...
	public List<NewsCat> list() {
		...
		String sql = "select * from NewsCat where isdeleted!=1";
		Object[] params = {};
	...
	public Long count() {
		...
		String sql = "select count(1) from NewsCat where isdeleted!=1";
		Object[] params = {};
	...
	public List<NewsCat> pager(Long pageNum, Long pageSize) {
		...
		sql = "select * from ( select rownum r,a.* from ( " + sql + ")a where rownum<=" + (startIndex+pageSize-1) + " ) b where r>=" + startIndex;
	...
	public NewsCat loadByName(String name) {
		...
		sql += " where catName=? and isdeleted!=1";
		Object[] params = { name };
	...
	public List<NewsCat> pagerByName(String name, Long pageNum, Long pageSize) {
		...
		// 分页
		Long startIndex = (pageNum - 1) * pageSize;
		sql = "select * from ( select rownum r,a.* from ( " + sql + ")a where rownum<=" + startIndex + " ) b where r>=" + (startIndex+pageSize-1);
